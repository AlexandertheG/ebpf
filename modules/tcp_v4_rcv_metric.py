...
class TcpV4RcvMetric:
    ...
    def __init__(self):
        ...
        int do_tcp_v4_rcv(struct pt_regs *ctx, struct __sk_buff *skb) {
        struct data_t data = {};
        ...
        self.bpf.attach_kprobe(event='tcp_v4_rcv', fn_name="do_tcp_v4_rcv")
        self.bpf["events"].open_perf_buffer(self.__count_tcp_v4_rcvs)
    ...
    def __count_tcp_v4_rcvs(self, cpu, data, size):
    ...
    def __get_prometheus_format(self):
        ...
        for k, v in self.aggregates.items():
            res = "tcp_v4_rcv{" + "ifindex=\"{}\"".format(k) + "} " + "{}".format(v)
            arr.append(res)
        return "\n".join(arr)
